...
def input_fn_spies(mocker):
    train_input_fn_spy = mocker.spy(supplying_datasets, 'train_input_fn')
    eval_input_fn_spy = mocker.spy(supplying_datasets, 'eval_input_fn')
    eval_with_excludes_fn_spy = mocker.spy(supplying_datasets, 'eval_with_excludes_fn')
...
def test_should_call_in_memory_evaluator_hooks(input_fn_spies,
    ...
    # assert fake_model.model_fn_calls == (3 if config.excluded_keys else 2)
...
@pytest.mark.integration
def test_should_train_with_each_model_during_experiment(mocker,
                                                        patched_read_dataset):
